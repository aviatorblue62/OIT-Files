...
var lastupdate = "6/29/2009, 7:09:17 PM";  // Created by Webetiser(tm) indexHTML 3.0 (Wine, Fast-hack 5), Thomas Almy
...
entry.item[50].keywords = "#123,#25,#AB1,#ABC,#BC7,#GHI,#PQR,#XYZ,$3000," +
                          "$8B,$9B,$AB,$BB,$ff,&ABC,&DEF,@AB1,@BC7,@PQR," +
                          "A-5,AB1,abbreviation,access,accomplished," +
                          "Accumulator,accumulators,Add,adda,addd,added," +
                          "addess,adding,addition,additional,address," +
                          "addresses,Addressing,advance,algorithms,allowed," +
                          "allowing,allows,appears,Appendix,applicable," +
                          "applies,arithmetic,array,arrays,assembler," +
                          "automatically,based,basic,BC7,behaves,best,bit," +
                          "bits,brackets,branch,branch/jump,byte,bytes," +
                          "calculate,calculated,calculates,calculation," +
                          "called,case,char,character,Classes,code,column," +
                          "comma,confusing,consists,Constant,constants," +
...
entry.item[73].keywords = "accurate,achieve,activity,addition,aditionally," +
                          ...
                          "phase-locked,picofarads,Pin,PLL,PLLON,Port," +
                          ...
                          "several,should,shown,shows,shut,signal,smallest," +
                          "source,sources,speed,stable,standard,start,stop," +
                          "stopping,stops,switched,switches,SYNR,System," +
                          "Text,Thus,time,timers,timing,Traps,turn,two," +
                          "undivided,use,used,User,Users,uses,using,value," +
                          "values,voltage,wai,Wait,waiting,won,Writing,XFC," +
                          "XTAL";
...
entry.item[82].keywords = "$0020,$0021,$00210020,$0021FFFC,$0022,$0022FFFD," +
                          "$FFFD,Accumulator,accumulators,accurate," +
                          "activity,addd,addition,additional,advantage," +
                          "allows,aren,assume,background,basic,bit,bits," +
                          "Block,bne,bold,buffers,bytes,called,cannot," +
                          "capable,Capture,captured,Case,cause,change," +
                          "changed,Channel,channels,check,circuit,clock," +
                          "clock-driven,clocks,CNTEXT,code,Compare," +
                          "conclusion,configuration,configured,connect," +
                          "Consider,consists,consumption,contents,control," +
                          "Count,counter,counters,cpd,cycles,data,DDRT," +
                          "debug,debugger,default,described,desirable," +
                          ...
                          "Users,using,value,values,variable,vector," +
                          "virtual,wai,wait,whenever,wide,width,widths," +
                          "word,worked,writing,written,wrong";
entry.item[82].page = "part018.html";
entry.item[82].description = "The Timer Module";
entry.item[83].keywords = "$01,$02,Accumulator,adca,addition,additional," +
                          "addressing,affects,allows,alter,applications," +
                          "assume,assuming,bclr,bit,bits,block,blue,bmi," +
                          "both,Branch,brclr,bset,C0F,C0I,C1F,C1I,C2F,C2I," +
                          "C3F,C3I,C4F,C4I,C5F,C5I,C6F,C6I,C7F,C7I," +
                          "Calculate,calculated,called,cannot,Capture," +
                          "Capture/Output,captured,captures,capturing,Care," +
                          "Case,change,changing,Channel,channels,chart," +
                          "Check,choice,clear,clears,clock,CNTEXT,code," +
                          "Compare,Compared,configuration,configure," +
                          "configured,Connect,connected,consecutive," +
                          "control,copy,correct,corrected,Count,counts,CPU," +
                          "cycles,data,decide,default,described,desired," +
                          "detected,detector,deteriorates,devoted,diagram," +
                          "difference,different,differing,disable,disabled," +
                          "doesn,driven,easy,EDG,EDG0A,EDG0B,EDG1A,EDG1B," +
                          "EDG2A,EDG2B,EDG3A,EDG3B,EDG4A,EDG4B,EDG5A,EDG5B," +
                          "EDG6A,EDG6B,EDG7A,EDG7B,edge,edges,efficiency," +
                          "eight,Elevating,enable,end,entered,entire,E-S," +
                          "examine,examples,execute,execution,explicitly," +
                          "extend,extended,Extending,external,falling," +
                          "fashion,feature,fetches,figure,fine,finishes," +
                          "flag,following,function,Get,gets,going,hardware," +
                          "having,HC12,help,high,higher,holds,HPRIO,ideal," +
                          "immediately,important,include,increase," +
                          "increment,incremented,increments,Index,Input," +
                          "Instead,instruction,instructions,interrupt," +
                          "interrupts,invalid,involved,inx,IOS0,IOS1,IOS2," +
                          "IOS3,IOS4,IOS5,IOS6,IOS7,key,know,known,large," +
                          "larger,LASTTC0,latter,ldd,ldx,leading,least," +
                          "Lets,levels,limit,limitation,limitations,loaded," +
                          ...
                          "move,nanoseconds,need,needed,needs,nega,negate," +
                          "Negative,negb,Next,Note,number,obtain,occurred," +
                          "occurs,operation,order,Output,overflow," +
                          ...
                          "used,uses,using,value,values,variable,variables," +
                          "version,Wait,wakeups,wanted,wave,waveform,way," +
                          "whenever,width,widths,word,work,world,writing";
...
entry.item[87].keywords = "#10,#11,#1199,#2399,#include,#RAMEND,#tc7int," +
                          ...
                          "counter,counts,covered,cycle,cycles,DBUG12," +
                          "D-BUG12,dedicated,described,discussed,drive," +
                          "driven,drives,duty,easy,emulator,enable,enabled," +
                          "entry,equals,executed,execution,feature," +
                          "following,frequency,function,Functionality," +
                          "generate,generated,generates,generating," +
                          "generator,half,HC12,HCS12,here,high,Idle," +
                          "important,inc,Increment,Index,initial," +
                          "Initialization,Initialize,Input,instance," +
                          "interrupt,interrupts,IOS,IOS0,IOS1,IOS2,IOS3," +
                          "IOS4,IOS5,IOS6,IOS7,kHz,large,ldd,lds,low,magic," +
                          "make,match,memory,MHz,microcontrollers," +
                          "microseconds,milliseconds,mode,modulated," +
                          "Modulation,Module,movw,necessary,never,Next,nop," +
                          "Note,number,OC0,OC7,OC7D,OC7D0,OC7D1,OC7D2," +
                          "OC7D3,OC7D4,OC7D5,OC7D6,OC7D7,OC7M,OC7M0,OC7M1," +
                          "OC7M2,OC7M3,OC7M4,OC7M5,OC7M6,OC7M7,occurs,OL7," +
                          "OM7,org,originally,Output,overflows,overhead," +
                          "period,pin,pins,point,pointer,Port,possible,PR0," +
                          "PR1,PR2,pressed,priority,process,processor," +
                          "program,programs,proportion,PRSTART,PT0,PT1,PT2," +
                          "PT4,PT5,PT7,Pulse,PWM,Questions,range,realize," +
                          "red,Register,Registers,rely,remains,require," +
                          "reset,resets,restrict,result,Return,Routine,rti," +
                          "run,save,Search,Section,Service,servos,sets," +
                          "signal,signals,simulator,simultaneously,single," +
                          "software,Special,specified,specify,spent,square," +
                          "stack,start,state,states,Status,std,symmetrical," +
                          "systems,tasks,TC0,TC7,tc7int,TCNT,TCRE,TCTL1," +
                          "TCTL2,TEN,Text,TFFCA,Thus,TIE,time,Timer,TIOS," +
                          "TMSK2,TOF,toggle,TOI,TSCR1,TSCR2,Turn,twice," +
                          "under,use,used,UserTimerCh7,uses,using,value," +
                          "values,varies,vector,wai,wave,waves,width,zero";
...
entry.item[130].keywords = "#-123,$00,$000,$0000,$00FF,$07,$10,$1000,$1001," +
                           ...
                           "inn,input,Input/Output,inputs,instead," +
                           "Instruction,Instructions,insure,Integer," +
                           "integers,Intel,intended,Interface,interfaced," +
                           "Interfaces,Interfacing,Inter-Integrated," +
                           "Internal,interpolation,interrupt,interruptable," +
